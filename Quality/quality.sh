...
Commands:
  ...
  source      Validate selected environment, build FormalMath, then run source/import/API-boundary and authored traceability integrity checks.
...
run_source() {
  run_recorded source bash -c \
    'bash Quality/check-environment.sh semantic && bash Quality/check-source-quality.sh production && lake build --wfail FormalMath && lake exe traceability validate'
}
