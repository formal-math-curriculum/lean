...
/-!
Deterministic validation for the authored M2.8 FART/FLOC/FLINK registry and its minimal curriculum
lock. The registry is Project-2 traceability authority; the curriculum lock is explicitly a
non-authoritative offline mirror of Project-1 release/lineage data.
-/
...
private def failE (context msg : String) : Except String α :=
  throw s!"traceability:error:{context}:{msg}"

private def objKeys (j : Json) : Except String (List String) := do
  let obj ← j.getObj?
  return obj.foldl (init := []) fun acc key _ => key :: acc
...
private def strArrayField (context : String) (j : Json) (key : String) : Except String (List String) := do
  ...
  for value in values do
    let s ← value.getStr? |>.mapError fun e => s!"{context}:{key}:{e}"
    result := s :: result
  return result.reverse

private def requireNonempty (context key value : String) : Except String Unit :=
  if value.isEmpty then failE context s!"empty-field:{key}" else pure ()
...
private def pad6 (n : Nat) : String :=
  let s := toString n
  String.mk (List.replicate (6 - s.length) '0') ++ s
...
private def readFamily (root : FilePath) (family : String) : IO (Array LocatedJson) := do
  ...
  for path in paths do
    if path.extension == some "jsonl" then
      ...
      for value in values do
        records := records.push { path, value }
  return records
...
private def validateManifest (j : Json) : Except String RegistryManifest := do
  ...
  if (← strField context j "registry_semantics_ref") != "P2-TRACE-M2.8-REGISTRY-v1" then
    failE context "registry-semantics-ref-mismatch"
  if (← strField context j "protocol_ref") != "P2-TRACE-M2.8-PROTOCOL-v1" then
    failE context "protocol-ref-mismatch"
  ...
  return { nextFart, nextFloc, nextFlink, countFart, countFloc, countFlink,
...
private def validateLockManifest (j : Json) : Except String LockManifest := do
  ...
  return { release, status, identityCount := (← natField context j "identity_count") }
...
private def validateLockIdentity (j : Json) : Except String LockIdentity := do
  ...
  let current ← strField context j "candidate_ref_current_resolved"
  requireNonempty context "candidate_ref_as_recorded" recorded
  requireNonempty context "candidate_ref_current_resolved" current
  let state ← strField context j "resolution_state"
...
private def reservationNumbers (prefix context : String) (reservations : List String) : Except String (List Nat) := do
  let mut result := []
  for rid in reservations do
    if rid.startsWith prefix then
      result := (← idNumber context prefix rid) :: result
  return result
