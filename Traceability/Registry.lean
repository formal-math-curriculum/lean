...
/-!
Deterministic validation for the authored M2.8 FART/FLOC/FLINK registry and the minimal
...
private def strArrayField (context : String) (j : Json) (key : String) : Except String (List String) := do
  ...
  for value in values do
    result := (← value.getStr? |>.mapError fun e => s!"traceability:error:{context}:{key}:{e}") :: result
  ...
  for path in paths do
    if path.extension == some "jsonl" then
      for value in (← readCanonicalJsonl path) do
        result := { path := path, value := value } :: result
  return result.reverse
...
private def validateManifest (j : Json) : Except String RegistryManifest := do
  ...
  let nextFart ← idNumber "registry-manifest.next_ids" "FART-P2-" (← strField "registry-manifest.next_ids" nextIds "fart")
  let nextFloc ← idNumber "registry-manifest.next_ids" "FLOC-P2-" (← strField "registry-manifest.next_ids" nextIds "floc")
  let nextFlink ← idNumber "registry-manifest.next_ids" "FLINK-P2-" (← strField "registry-manifest.next_ids" nextIds "flink")
  ...
  return {
    nextFart := nextFart, nextFloc := nextFloc, nextFlink := nextFlink,
    countFart := countFart, countFloc := countFloc, countFlink := countFlink,
...
private def validateSourceProvenance (j : Json) : Except String Unit := do
  ...
  requireSortedUnique context "source_refs" (← strArrayField context j "source_refs")
...
private def validateFart (located : LocatedJson) : Except String FartSummary := do
  ...
  if located.path.fileName.getD "" != expectedShard number then failE context s!"wrong-shard:{id}"
  ...
  let locs ← strArrayField context j "current_locator_refs"
  let links ← strArrayField context j "curriculum_link_refs"
  let supersedes ← strArrayField context j "supersedes"
  let supersededBy ← strArrayField context j "superseded_by"
  requireSortedUnique context "current_locator_refs" locs
  requireSortedUnique context "curriculum_link_refs" links
  requireSortedUnique context "supersedes" supersedes
  requireSortedUnique context "superseded_by" supersededBy
  for ref in locs do let _ ← idNumber context "FLOC-P2-" ref
  for ref in links do let _ ← idNumber context "FLINK-P2-" ref
  ...
  return { id := id, number := number, currentLocators := locs, curriculumLinks := links }
...
private def validateFloc (located : LocatedJson) : Except String FlocSummary := do
  ...
  let declarations ← strArrayField context j "declaration_names"
  ...
  requireSortedUnique context "declaration_names" declarations
  ...
  requireNonempty context "created_revision" (← strField context j "created_revision")
  return { id := id, number := number, fart := fart, locatorStatus := locatorStatus, sourceKind := sourceKind, filePath := filePath }
...
private def validateFlink (located : LocatedJson) : Except String FlinkSummary := do
  ...
  requireEnum context "record_status" (← strField context j "record_status") ["active", "historical", "superseded", "retired", "unresolved"]
  requireNonempty context "created_revision" (← strField context j "created_revision")
  return { id := id, number := number, fart := fart, candidateRecorded := candidateRecorded, linkStatus := linkStatus, lineageState := lineageState }

private def validateLockManifest (j : Json) : Except String LockManifest := do
  let context := "curriculum-lock-manifest"
  requireExactKeys context j ["authority", "curriculum_release_ref", "identity_count", "mirror_status", "schema_version", "source_refs", "verified_by_trace_record"]
  if (← natField context j "schema_version") != 1 then failE context "unsupported-schema-version"
...
private def validateLockIdentity (j : Json) : Except String LockIdentity := do
  ...
  requireExactKeys context j ["candidate_ref_as_recorded", "candidate_ref_current_resolved", "record_status", "resolution_path", "resolution_state", "schema_version", "treatment_scopes"]
  if (← natField context j "schema_version") != 1 then failE context "unsupported-schema-version"
...
private def validateIssuedBounds (family idPrefix : String) (nextId : Nat) (records : List LocatedJson) : Except String Unit := do
  for located in records do
    let id ← strField s!"allocation.{family}" located.value "id"
    if (← idNumber s!"allocation.{family}" idPrefix id) >= nextId then
...
private def validateReservationBounds (manifest : RegistryManifest) : Except String Unit := do
  for rid in manifest.reservations do
    if rid.startsWith "FART-P2-" then
      if (← idNumber "registry.reservation" "FART-P2-" rid) >= manifest.nextFart then failE "allocation" s!"reserved-id-not-below-next:{rid}:next={manifest.nextFart}"
    else if rid.startsWith "FLOC-P2-" then
      if (← idNumber "registry.reservation" "FLOC-P2-" rid) >= manifest.nextFloc then failE "allocation" s!"reserved-id-not-below-next:{rid}:next={manifest.nextFloc}"
    else if rid.startsWith "FLINK-P2-" then
      if (← idNumber "registry.reservation" "FLINK-P2-" rid) >= manifest.nextFlink then failE "allocation" s!"reserved-id-not-below-next:{rid}:next={manifest.nextFlink}"

private def validateCrossReferences
    (manifest : RegistryManifest) (farts : List FartSummary) (flocs : List FlocSummary)
    (links : List FlinkSummary) (lock : LockManifest) (lockIds : List LockIdentity) : Except String Unit := do
  ...
  for link in links do
    ...
    if !(lockIds.any (fun x => x.recorded == link.candidateRecorded)) then
      ...
      if !explicitNonSuccess then failE "curriculum-lock" s!"missing-linked-identity:{link.id}:{link.candidateRecorded}"
...
public def validateRegistryRoot (root : FilePath := ".") : IO Unit := do
  ...
  IO.ofExcept <| validateIssuedBounds "fart" "FART-P2-" manifest.nextFart fartRaw
  IO.ofExcept <| validateIssuedBounds "floc" "FLOC-P2-" manifest.nextFloc flocRaw
  IO.ofExcept <| validateIssuedBounds "flink" "FLINK-P2-" manifest.nextFlink flinkRaw
  IO.ofExcept <| validateReservationBounds manifest

  let mut farts := []
  for record in fartRaw do farts := (← IO.ofExcept <| validateFart record) :: farts
  farts := farts.reverse
  let mut flocs := []
  for record in flocRaw do
    let loc ← IO.ofExcept <| validateFloc record
    if loc.locatorStatus == "current" && loc.sourceKind == "project_repository" then
      if !(← (root / loc.filePath).pathExists) then failIO s!"traceability:error:floc:missing-current-project-file:{loc.id}:{loc.filePath}"
    flocs := loc :: flocs
  flocs := flocs.reverse
  let mut links := []
  for record in flinkRaw do links := (← IO.ofExcept <| validateFlink record) :: links
  links := links.reverse

  let lock ← IO.ofExcept <| validateLockManifest (← readCanonicalJson (root / "metadata" / "curriculum-lock" / "manifest.json"))
  let lockRaw ← readCanonicalJsonl (root / "metadata" / "curriculum-lock" / "linked-identities.jsonl")
  let mut lockIds := []
  for record in lockRaw do lockIds := (← IO.ofExcept <| validateLockIdentity record) :: lockIds
